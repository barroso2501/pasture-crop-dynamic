...
- **Status:** Approved; Phases 0–8 complete and accepted; Phase 9 synthesis is next
- **Date recorded:** 2026-09-12
- **Last implementation update:** 2026-09-21 — Phase 8E completed; Phase 8 closed
...
Phase 8A was completed and accepted. It constructs three
...
Phase 8A calculates geometry and support only. It reports the support-fraction
distribution without using process outcomes to select units. Decision 014
governs the fixed-domain and alternative-grid rules. The accepted
positive-support populations are 56,520 cells for `hex10k_base`, 29,396 for
`hex20k_shift` and 15,309 for `hex40k_base`.

### Phase 8B implementation outcome

Phase 8B was completed and accepted on 2026-09-19. Consolidation,
replenishment and endpoint `NAT → TMP` areas were recomputed directly from the
source rasters on every alternative grid for all eight intervals. Derived net
balance, bounded C–R balance, process-specific rates and fixed support flags
were assembled in an 809,800-row panel.

The Earth Engine extraction uses a common raster-support mask for the fixed
domain and both endpoint observation partitions. The accepted validator is
revision 4. It requires temporal support invariance within each grid,
agreement with the fixed vector footprint, bounded cross-grid boundary
variation and outcome-independent comparison tolerances calculated separately
for each grid and interval. Decision 015 records these rules.

All 14 validation checks passed. The maximum within-grid temporal support
spread was `3.9618e-14`, the maximum cross-grid spread was `6.2993e-6`, and the
maximum vector–raster support difference was `0.000426549`, below the `0.002`
geometric limit. All 72 canonical-total comparisons passed. Maximum relative
differences were `0.000188116` for consolidation, `0.000055211` for
replenishment and `0.000011506` for endpoint `NAT → TMP`.

Phase 8B is a measurement and validation stage. It does not itself establish
substantive MAUP robustness. The next stage will compare distributions,
temporal conclusions and selected spatial statistics across the canonical and
alternative grids using prespecified criteria. The canonical grid remains the
primary analysis.
...
docs/decisions/015_phase8b_geometric_support_and_total_tolerance.md
docs/methods/maup_alternative_grid_core_metrics.md
docs/validation/canonical_maup_core_metrics_phase8b.md
outputs/validation/phase8b_maup_core_metrics_v1/
```

### Phase 8C implementation outcome

Phase 8C was completed and accepted on 2026-09-20. Decision 016 froze the
comparison metrics, support scopes, primary and full-observed windows, and
substantive stability thresholds before inspection of the alternative-grid
results. The accepted script harmonized 199,112 canonical and 809,800
alternative-grid records.

All 192 aggregate comparisons and all 48 temporal comparisons passed. No
aggregate sign or C-R dominance class changed. Every temporal Spearman
correlation was `1.0`, and every peak and trough occurred in the same interval
as on the canonical grid. The diagnostic 2020-2025 extension changed no
grid-level assessment.

The shifted approximately 20,000-ha grid passed every primary criterion in
both windows, supporting robustness to lattice origin at the canonical scale.
The approximately 10,000-ha grid was classified as sensitive because one
1985-1990 defined-support comparison exceeded the 0.05 limit by 0.000537; its
associated KS distance passed. This is retained as a strict but marginal
sensitivity result.

The approximately 40,000-ha grid showed systematic scale sensitivity in the
defined support of the C-R balance index and consolidation rate. It also
slightly exceeded the weighted KS limit for `NAT -> TMP` density and intensity
in 2015-2020 and diagnostic 2020-2025. These findings affect cell-distribution
and conditional-support interpretation but do not reverse aggregate or
temporal conclusions.

Phase 8C therefore accepts the canonical approximately 20,000-ha grid as the
primary support while requiring scale qualifications for conditional coverage,
typical-cell summaries and recent `NAT -> TMP` distributions. Validation passed
because sensitivity findings are scientific outcomes, not runtime failures.

At the conclusion of Phase 8C, the remaining spatial-configuration components
were the alternative-grid Global Moran comparison and a broad evaluation of
cluster location or persistence. Phase 8C did not construct alternative
neighbor graphs or recompute Local Moran statistics. The Global Moran
component is addressed by Phase 8D below.

Evidence and implementation are recorded in:

```text
analysis/14a_compare_maup_distributions_and_temporal_patterns.py
docs/decisions/016_phase8c_maup_comparison_criteria.md
docs/methods/maup_distribution_and_temporal_comparison_phase8c.md
docs/validation/canonical_maup_distribution_temporal_comparison_phase8c.md
docs/analysis/phase8c_maup_robustness_findings_v1.md
outputs/validation/phase8c_maup_comparison_v1/
figures/phase8c_maup_comparison_v1/
```

### Phase 8D implementation outcome

Phase 8D was completed and accepted on 2026-09-20. Decision 017 froze the
alternative-grid support rule, axial contiguity construction, five focal
metrics, permutation inference, multiplicity families and substantive
comparison thresholds before result inspection. The accepted revision 2
script differs from revision 1 only by converting a non-applicable canonical
graph diagnostic to the declared numeric sentinel `0.0`; it reused the completed statistical
checkpoints without recalculation.

The three alternative graphs contain 49,716 cells for `hex10k_base`, 25,015
for `hex20k_shift` and 12,445 for `hex40k_base`, using domain support fraction
`>= 0.50`. All 120 alternative-grid Global Moran tests used 9,999
permutations. Every coefficient was positive and remained significant after
the prespecified Benjamini-Yekutieli correction. Sign and significance were
therefore preserved in all interval-level comparisons.

Of 120 magnitude comparisons, 119 passed the absolute-difference limit of
0.10. The single failure was endpoint `NAT -> TMP` density on the
approximately 40,000-ha grid in 2005-2010: canonical `I = 0.405113`,
alternative `I = 0.507941`, and absolute difference `0.102828`.

Eight of 30 temporal comparisons failed the strict conjunction of Spearman
correlation `>= 0.90` and peak/trough displacement no greater than one
interval. Five failures concerned endpoint `NAT -> TMP`; three concerned the
C-R balance index, including both 10,000-ha windows and the full-observed
40,000-ha window. The shifted 20,000-ha `NAT -> TMP` correlations remained
above 0.95, but its peak moved by two intervals. The diagnostic 2020-2025
interval was included and flagged.

Because the aggregate rule requires every interval and temporal criterion to
pass, all six grid-window assessments are formally classified as sensitive
for at least one criterion. This does not imply loss of spatial
autocorrelation. Phase 8D supports robustness of the positive direction and
formal significance of global clustering, with partial MAUP sensitivity in
exact magnitudes and temporal ordering, concentrated in endpoint
`NAT -> TMP`.

The canonical approximately 20,000-ha grid remains the primary support.
The remaining broad comparison of cluster location and persistence is
addressed by Phase 8E below. Alternative-grid results do not replace the
canonical Local Moran/LISA analysis.

Evidence and implementation are recorded in:

```text
analysis/14b_compute_maup_global_moran_v2.py
docs/decisions/017_phase8d_maup_global_moran_criteria.md
docs/methods/maup_global_moran_phase8d.md
docs/validation/canonical_maup_global_moran_phase8d.md
docs/analysis/phase8d_maup_global_moran_findings_v1.md
outputs/validation/phase8d_maup_global_moran_v1/
figures/phase8d_maup_global_moran_v1/
```

### Phase 8E implementation outcome

Phase 8E was completed and accepted on 2026-09-21. Decision 018 froze the
primary HH cluster, support rule, permutation inference, multiplicity families,
polygon-intersection crosswalk and location and persistence thresholds before
the full alternative-grid results were inspected. The accepted script computed
120 Local Moran maps with 9,999 conditional permutations and retained
3,487,040 cell-map records. BH at 5% is the primary inference; BY remains a
sensitivity analysis.

The three exact polygon crosswalks contain 136,201, 93,843 and 67,245 rows.
Their relative area-closure errors are at most `2.522e-16`. All expected maps,
240 interval comparisons, 60 temporal comparisons and 30 primary robustness
assessments are present. The execution validation passed, and an independent
review reproduced the compact-table counts, interval decisions, temporal
decisions and overall assessment rules without discrepancies.

Twenty-one of 30 grid–metric–window assessments passed. Replenishment, net
C–R balance and the bounded C–R index passed all six assessments per metric.
Consolidation passed three of six: its recurrence and persistence tests pass,
but several interval footprints fail the broad-location criterion, especially
on the 10,000-ha grid. Endpoint `NAT -> TMP` passed none of six overall
assessments and only seven of 24 primary interval comparisons. Its persistent
HH footprints also fail for the 10,000-ha and shifted 20,000-ha grids. The
40,000-ha temporal persistence comparisons pass, but too few interval maps
meet the location rule.

Many failed interval comparisons retain high overlap coefficients while
Jaccard similarity is low. This indicates contraction or expansion around a
shared geographic core rather than wholesale relocation. It does not satisfy
the prespecified rule because the affected footprint sizes differ materially.
The diagnostic 2020–2025 interval reinforces but does not create the principal
`NAT -> TMP` sensitivity. It additionally causes the full-observed 40,000-ha
consolidation assessment to fall below its required six interval passes.

BY results remain secondary. Twenty-three of 120 BY interval comparisons have
empty HH footprints on both supports and therefore receive a mathematical
Jaccard of 1.0; these cases are not interpreted as evidence of strong
geographic agreement.

Phase 8 is now closed. The canonical approximately 20,000-ha grid remains the
primary support. The synthesis must report robust HH location and persistence
for replenishment and the C–R measures, partial robustness for consolidation,
and clear MAUP sensitivity for endpoint `NAT -> TMP`. Successful validation
means that the planned comparison was completed consistently; it does not mean
that every substantive robustness criterion passed.

Evidence and implementation are recorded in:

```text
analysis/14c_compare_maup_hh_clusters_v1.py
docs/decisions/018_phase8e_hh_cluster_location_and_persistence.md
docs/methods/maup_hh_cluster_robustness_phase8e.md
docs/validation/canonical_maup_hh_cluster_robustness_phase8e.md
docs/analysis/phase8e_maup_hh_cluster_findings_v1.md
docs/analysis/phase8_maup_robustness_synthesis_v1.md
outputs/validation/phase8e_maup_hh_cluster_v1/
figures/phase8e_maup_hh_cluster_v1/
```
