...
* **Status:** Accepted. The MapBiomas Collection 11 temporal-filter specification is recorded for TMP, PAS, and NAT, including the PAS edge-year rules and the formal source citations. The former specification and source-recording conditions are satisfied. Repository-wide implementation of the reporting-language and caption requirements remains subject to audit; see Acceptance criteria.
...
* **Related methods doc:** `docs/methods/data_sources_and_classification.md` (defines the `NAT`/`PAS`/`TMP` MapBiomas code groups referenced in the Filter specification section)
...
* **right-boundary-censored**: the fate beyond 2025 is unknown for episodes or trajectories still ongoing at the last observation (Decision 009); this applies to claims about persistence, duration, reversal, or completion, not to simple endpoint-composition claims, and is not a classification-quality property of every observation made during 2020–2025;
...
* **2020–2023** `NAT→TMP` and `PAS→TMP` claims are not subject to the temporal-filter degradation described here for TMP's temporary-crops component or for PAS. MAUP sensitivity still applies to local-cluster claims. Right-boundary censoring applies only when an episode or trajectory observed during this sub-window remains ongoing through 2025.
...
Rejected. Sections 1, 2, 4, and 5, and the qualitative part of Section 3, did not depend on that specification. Following the precedent of Decision 020, the decision was initially approved with an explicit open condition. The specification and source citations have since been incorporated in this version, satisfying that former condition without altering any accepted computation or output.
...
The filter-specification and source-recording criteria are satisfied as of 2026-09-23. The repository-wide reporting-language and caption criteria remain to be verified through a documentation and manuscript audit. No accepted computation, cohort definition, partition, or validated numerical output is changed by this decision.
...
* **right-boundary-censored** (episode/trajectory-fate property, per Decision 009; applies when an episode or trajectory remains ongoing at the last observation in 2025, not uniformly to every observation within the interval);
