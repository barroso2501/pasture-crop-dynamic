...
The canonical workflow has been reproduced and validated through construction of the alternative spatial units for the MAUP analysis. It includes the study domain, five-year stock-and-flow and annual `NAT → TMP` trajectory panels, integrated cell-by-interval metrics, fixed spatial support and contiguity graph, comparable map classes, cell-state trajectories, temporal synthesis, Global Moran, Local Moran/LISA, biome-specific comparisons, and three accepted alternative hexagonal grids. Phases 1–7 and Phase 8A are complete. Phase 8B will recompute a restricted set of process metrics on the alternative grids.
...
Phase 8A constructed and validated a 10,000-ha base grid (56,520 cells), a 20,000-ha grid shifted by half of each canonical lattice basis vector (29,396 cells), and a 40,000-ha base grid (15,309 cells). All three close to the same 497.7764-Mha footprint formed by the union of the canonical cells. Phase 8A calculated geometry and support only. The next stage is Phase 8B, which will measure the restricted process set directly from source pixels; canonical-cell totals will not be spatially interpolated onto the alternative grids.
