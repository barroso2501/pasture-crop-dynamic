...
The analytical workflow and documentation are being rebuilt and validated. The canonical study domain, five-year stock-and-flow panel, derived stock-flow metrics, and annual `NAT → TMP` within-interval trajectory panel have now been reproduced and passed their predefined computational and accounting checks. The two analytical panels have also been integrated into a validated cell-by-interval table. A runtime-provenance regression test confirmed that the accepted Earth Engine outputs used the native Collection 11 coverage lattice. The canonical spatial-support table, biome-overlap attributes, fixed shared-edge contiguity graph, and process-specific spatial metrics have also been constructed and validated. Phase 3A established the comparable-class framework and a minimum-support rule for cell-level trajectory shares; its final share-limit refit remains pending. Comparable mapping, spatial interpretation, and robustness analyses remain in development.
...
**Current status: canonical inputs and analytical domain validated; the complete 1985–2025 five-year stock-and-flow series and `NAT → TMP` within-interval trajectory series have been reprocessed, assembled, validated, and integrated into a canonical 199,112-row analytical panel. The runtime-provenance gate passed without requiring reprocessing. Phase 1 produced a validated 24,889-cell spatial table and a fixed graph with 67,453 shared-edge links. Phase 2 joined this support to the integrated panel and validated process-specific spatial metrics. The Phase 3A diagnostic accepted ten comparable-class specifications and established a 0.1%-of-cell minimum endpoint support for the two trajectory-share maps. Those two limits must now be refitted before Phase 3B map production.**
