...
CLASS_LIMITS_OUTPUT = OUTPUT_DIR / "canonical_comparable_map_class_limits_v2.csv"
DISTRIBUTIONS_OUTPUT = (
    OUTPUT_DIR / "canonical_comparable_map_distributions_v2.csv"
)
CLASS_COUNTS_OUTPUT = OUTPUT_DIR / "canonical_comparable_map_class_counts_v2.csv"
SHARE_DENOMINATOR_SENSITIVITY_OUTPUT = (
    OUTPUT_DIR / "canonical_share_denominator_sensitivity_v2.csv"
)
SHARE_DENOMINATOR_SENSITIVITY_POOLED_OUTPUT = (
    OUTPUT_DIR / "canonical_share_denominator_sensitivity_pooled_v2.csv"
)
SHARE_SUPPORT_SUMMARY_OUTPUT = (
    OUTPUT_DIR / "canonical_trajectory_share_support_summary_v2.csv"
)
VALIDATION_OUTPUT = OUTPUT_DIR / "canonical_map_classes_validation_v2.json"
...
OUTPUT_VERSION = "canonical-comparable-map-classes-v2"
...
def share_classification_masks(
    frame: pd.DataFrame,
    specification: dict,
) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    """Return mathematical support, low support, eligibility, and threshold."""
    mathematically_defined = support_mask(frame, specification)
    minimum_support_ha = (
    ...
    eligible = (
        ...
        .gt(minimum_support_ha)
    )
    low_support = mathematically_defined & ~eligible
    return mathematically_defined, low_support, eligible, minimum_support_ha
...
print("CANONICAL COMPARABLE MAP CLASSES - PHASE 3A FINAL VERSION 2")
...
require(panel["geometry_area_aea_ha"].notna().all(),
        "Missing AEA cell area")
require((panel["geometry_area_aea_ha"] > 0).all(),
        "Nonpositive AEA cell area")
require(
    panel.groupby("cell_id")["geometry_area_aea_ha"].nunique().le(1).all(),
    "AEA cell area varies among intervals",
)
...
for specification in POSITIVE_METRICS:
    metric = specification["metric"]
    tolerance = specification["zero_tolerance"]
    mathematically_defined = support_mask(primary, specification)
    if metric in SHARE_METRIC_NAMES:
        _, low_support, classification_eligible, _ = share_classification_masks(
        ...
        classification_method = (
            "pooled_positive_median_and_p90_with_0p1pct_cell_support"
        )
        threshold_source = (
            "seven_primary_intervals_1985_2020_endpoint_gt_0p1pct_cell"
        )
        support_description = (
            f"{specification['support_field']}; "
            "nat_tmp_endpoint_ha > 0.001 * geometry_area_aea_ha"
        )
    else:
        low_support = pd.Series(False, index=primary.index)
        classification_eligible = mathematically_defined
        labels = [
            "undefined",
            "zero",
            "positive_low",
        ...
        support_description = specification["support_field"] or "all_rows"

    defined_values = pd.to_numeric(
        primary.loc[mathematically_defined, metric], errors="raise"
    )
    source_values = pd.to_numeric(
        primary.loc[classification_eligible, metric], errors="raise"
    )
    ...
    limit_rows.append(
        {
            ...
            "support_field": support_description,
            "minimum_share_support_fraction_of_cell": (
                SHARE_MINIMUM_CELL_FRACTION
                if metric in SHARE_METRIC_NAMES
                else "not_applicable"
            ),
            ...
            "source_defined_count": len(defined_values),
            "source_undefined_count": len(primary) - len(defined_values),
            "source_low_support_count": int(low_support.sum()),
            "source_classification_eligible_count": len(source_values),
...
limit_rows.append(
    {
        ...
        "minimum_share_support_fraction_of_cell": "not_applicable",
        ...
        "source_low_support_count": 0,
        "source_classification_eligible_count": len(signed_source),
...
limit_rows.append(
    {
        ...
        "source_low_support_count": 0,
        "source_classification_eligible_count": int(
            balance_primary_supported.sum()
        ),
...
require(
    (class_limits["source_low_support_count"]
     + class_limits["source_classification_eligible_count"])
    .eq(class_limits["source_defined_count"]).all(),
    "Low-support and classification-eligible source counts do not close",
)
require(
    (class_limits["source_zero_count"]
     + class_limits["source_nonzero_count"])
    .eq(class_limits["source_classification_eligible_count"]).all(),
...
for (t0, t1), interval_frame in panel.groupby(["t0", "t1"], sort=True):
    ...
    for specification in specifications:
        ...
        if metric in {item["metric"] for item in POSITIVE_METRICS}:
            if metric in SHARE_METRIC_NAMES:
                mathematically_defined, low_support, supported, _ = (
                    share_classification_masks(interval_frame, specification)
                )
            else:
                mathematically_defined = support_mask(
                    interval_frame, specification
                )
                low_support = pd.Series(False, index=interval_frame.index)
                supported = mathematically_defined
            ...
            if metric in SHARE_METRIC_NAMES:
                classes.loc[~mathematically_defined] = "not_applicable"
...
# -----------------------------------------------------------------------------
# 8. Final cell-relative trajectory-share support
# -----------------------------------------------------------------------------

share_support_rows = []


def append_share_support_row(
    frame: pd.DataFrame,
    specification: dict,
    scope: str,
    t0: int,
    ...
    limits = limit_lookup[metric]
    mathematically_defined, low_support, eligible, minimum_support_ha = (
        share_classification_masks(frame, specification)
    )
    ...
    endpoint_total = float(
        frame.loc[mathematically_defined, "nat_tmp_endpoint_ha"].sum()
    )
    endpoint_low_support = float(
        frame.loc[low_support, "nat_tmp_endpoint_ha"].sum()
    )
    endpoint_eligible = float(frame.loc[eligible, "nat_tmp_endpoint_ha"].sum())
    trajectory_total = float(
        frame.loc[mathematically_defined, numerator_field].sum()
    )
    trajectory_low_support = float(frame.loc[low_support, numerator_field].sum())
    trajectory_eligible = float(frame.loc[eligible, numerator_field].sum())
    eligible_positive = values.loc[
        eligible & values.gt(limits["zero_tolerance"])
    ]

    share_support_rows.append(
        {
            "metric": metric,
            "scope": scope,
            "t0": int(t0),
            ...
            "interval": (
                ...
                if scope == "interval"
                else "pooled_primary_1985_2020"
            ),
            "diagnostic_interval": diagnostic_flag,
            "support_fraction_of_cell": SHARE_MINIMUM_CELL_FRACTION,
            "minimum_support_ha_min": float(minimum_support_ha.min()),
            "minimum_support_ha_mean": float(minimum_support_ha.mean()),
            "minimum_support_ha_max": float(minimum_support_ha.max()),
            "total_observation_count": len(frame),
            "not_applicable_count": int(counts.get("not_applicable", 0)),
            "low_support_count": int(counts.get("low_support", 0)),
            "eligible_count": int(eligible.sum()),
            "eligible_zero_count": int(counts.get("zero", 0)),
            "eligible_positive_low_count": int(counts.get("positive_low", 0)),
            "eligible_positive_moderate_count": int(
                counts.get("positive_moderate", 0)
            ),
            "eligible_positive_high_count": int(counts.get("positive_high", 0)),
            "endpoint_area_total_ha": endpoint_total,
            "endpoint_area_low_support_ha": endpoint_low_support,
            "endpoint_area_eligible_ha": endpoint_eligible,
            "endpoint_area_eligible_fraction": (
                endpoint_eligible / endpoint_total
                if endpoint_total > AREA_ZERO_TOLERANCE_HA
                else np.nan
            ),
            "trajectory_area_total_ha": trajectory_total,
            "trajectory_area_low_support_ha": trajectory_low_support,
            "trajectory_area_eligible_ha": trajectory_eligible,
            "trajectory_area_eligible_fraction": (
                trajectory_eligible / trajectory_total
                if trajectory_total > AREA_ZERO_TOLERANCE_HA
                else np.nan
            ),
            "aggregate_share_complete": (
                trajectory_total / endpoint_total
                if endpoint_total > AREA_ZERO_TOLERANCE_HA
                else np.nan
            ),
            "aggregate_share_eligible": (
                trajectory_eligible / endpoint_eligible
                if endpoint_eligible > AREA_ZERO_TOLERANCE_HA
                else np.nan
            ),
            "eligible_positive_median": (
                float(
                    eligible_positive.quantile(
                        0.50, interpolation=QUANTILE_METHOD
                    )
                )
                if len(eligible_positive)
                else np.nan
            ),
            "eligible_positive_p90": (
                float(
                    eligible_positive.quantile(
...
share_specifications = [
    item for item in POSITIVE_METRICS if item["metric"] in SHARE_METRIC_NAMES
]
for (t0, t1), interval_frame in panel.groupby(["t0", "t1"], sort=True):
    for specification in share_specifications:
        append_share_support_row(
            interval_frame,
            specification,
            "interval",
            int(t0),
            int(t1),
            int((int(t0), int(t1)) == DIAGNOSTIC_INTERVAL),
        )
...
share_support_summary = pd.DataFrame(share_support_rows).sort_values(
    ["metric", "scope", "t0", "t1"]
).reset_index(drop=True)
require(len(share_support_summary) == 18,
        "Unexpected final share-support summary row count")
require(
    (
        share_support_summary["not_applicable_count"]
        + share_support_summary["low_support_count"]
        + share_support_summary["eligible_count"]
    ).eq(share_support_summary["total_observation_count"]).all(),
    "Final share-support populations do not close",
)
require(
    np.allclose(
        share_support_summary["endpoint_area_low_support_ha"]
        + share_support_summary["endpoint_area_eligible_ha"],
        share_support_summary["endpoint_area_total_ha"],
        rtol=0,
        atol=2e-6,
    ),
    "Final share-support endpoint areas do not close",
)
require(
    np.allclose(
        share_support_summary["trajectory_area_low_support_ha"]
        + share_support_summary["trajectory_area_eligible_ha"],
        share_support_summary["trajectory_area_total_ha"],
        rtol=0,
        atol=2e-6,
    ),
    "Final share-support trajectory areas do not close",
)

pooled_final_support = share_support_summary.loc[
    share_support_summary["scope"].eq("pooled_primary")
].set_index("metric")
for metric in SHARE_METRIC_NAMES:
    limit_row = class_limits.loc[class_limits["metric"].eq(metric)].iloc[0]
    support_row = pooled_final_support.loc[metric]
    require(
        int(limit_row["source_low_support_count"])
        == int(support_row["low_support_count"]),
        f"Pooled low-support count mismatch for {metric}",
    )
    require(
        int(limit_row["source_classification_eligible_count"])
        == int(support_row["eligible_count"]),
        f"Pooled eligible count mismatch for {metric}",
    )
    require(
        abs(float(limit_row["break_1"])
            - float(support_row["eligible_positive_median"]))
        <= NUMERIC_TOLERANCE,
        f"Pooled median mismatch for {metric}",
    )
    require(
        abs(float(limit_row["break_2"])
            - float(support_row["eligible_positive_p90"]))
        <= NUMERIC_TOLERANCE,
        f"Pooled p90 mismatch for {metric}",
    )


# -----------------------------------------------------------------------------
# 9. Trajectory-share denominator sensitivity
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 10. Export compact records and validation
# -----------------------------------------------------------------------------
...
share_support_summary.to_csv(
    SHARE_SUPPORT_SUMMARY_OUTPUT, index=False, float_format="%.12g"
)
...
validation = {
    ...
    "configuration": {
        ...
        "trajectory_share_rule": (
            "not applicable when NAT-TMP endpoint is absent; low support when "
            "endpoint <= 0.001 * AEA cell area; eligible zero; eligible "
            "positive low, moderate, and high from eligible pooled limits"
        ),
        "trajectory_share_minimum_cell_fraction": SHARE_MINIMUM_CELL_FRACTION,
        "quantile_method": QUANTILE_METHOD,
    ...
    "structure": {
        ...
        "final_share_support_summary_rows": len(share_support_summary),
    ...
    "checks": {
        ...
        "cell_relative_share_support_applied": True,
        "share_support_populations_close": True,
        "share_support_area_accounting_closes": True,
        "final_share_breaks_reconcile_with_eligible_primary_population": True,
...
print(
    class_limits[
        [
            ...
            "source_low_support_count",
            "source_classification_eligible_count",
            "source_zero_count",
...
print("\nFinal pooled trajectory-share support:")
print(
    pooled_final_support[
        [
...
print("Final trajectory-share support:", SHARE_SUPPORT_SUMMARY_OUTPUT)
