...
def load_inputs(project):
    ...
    columns = list(dict.fromkeys(KEYS + METRICS + CONTEXT + ["cr_balance_defined", "cr_balance_state"]))
...
def build_products(panel, limits, output):
    ...
    for item in inventory:
        ...
        lines += [f"[{path.name}]", "Format=CSVDelimited", "ColNameHeader=True",
                  "CharacterSet=65001", "MaxScanRows=0"]
...
def main():
    ...
    print("CANONICAL CELL TRAJECTORIES — PHASE 4A VERSION 1", flush=True)
